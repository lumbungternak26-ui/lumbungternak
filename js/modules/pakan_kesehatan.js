...
const PakanKesehatanModule = {
    ...
    renderStokPakanHPP() {
        ...
        const hppData = Store.calculateRealtimeHPP 
            ...
            : { hppPerEkorHari: 4900, totalHppHarianKandang: 4900 * dombaList.length, biayaKonsentrat: 2100, biayaSilase: 1800, biayaHijauan: 1000 };
        const fcrData = Store.calculateFCR 
            ? Store.calculateFCR(30) 
            : { fcr: 5.5, statusFcr: "Efisiensi Baik", fcrDesc: "Pertumbuhan stabil", totalPakanKg: 0, totalGainKg: 0 };
            ...
            <div class="space-y-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm">
                    ...
                    <div>
                        <button onclick="PakanKesehatanModule.openModalTambahPakan()" class="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl bg-teal-600 hover:bg-teal-700 text-white shadow-md transition active:scale-95">
                            <i data-lucide="plus-circle" class="w-4 h-4"></i> Tambah / Restok Pakan
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div class="bg-gradient-to-br from-teal-700 to-emerald-800 text-white p-5 rounded-2xl shadow-md space-y-1">
                        ...
                        <div class="text-2xl font-black">Rp ${hppData.hppPerEkorHari.toLocaleString('id-ID')}</div>
                        <p class="text-[11px] text-teal-100/80 truncate">
                            Kons: Rp ${hppData.biayaKonsentrat} • Sil: Rp ${hppData.biayaSilase} • Odot: Rp ${hppData.biayaHijauan}
                        </p>
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
                        ...
                        <div class="text-2xl font-black text-slate-900 dark:text-white">Rp ${hppData.totalHppHarianKandang.toLocaleString('id-ID')}</div>
                    ...
                    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
                        ...
                        <div class="text-2xl font-black text-teal-700 dark:text-teal-400">${fcrData.fcr}</div>
                        <p class="text-[11px] text-slate-500 truncate" title="${fcrData.fcrDesc}">${fcrData.fcrDesc}</p>
                    </div>

                    <!-- Card 4: Estimasi Bulanan -->
                    <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-1">
                        <span class="text-xs text-slate-500 font-medium">Estimasi Biaya Pakan (1 Bln)</span>
                        <div class="text-2xl font-black text-blue-700 dark:text-blue-400">Rp ${(hppData.totalHppHarianKandang * 30).toLocaleString('id-ID')}</div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs text-left">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                ${stokPakan.length === 0 ? `
                                    <tr>
                                        <td colspan="7" class="text-center py-6 text-slate-400">Tidak ada komoditas pakan yang cocok.</td>
                                    </tr>
                                ` : stokPakan.map(s => `
                                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-900/30">
                                        <td class="py-3 px-4 font-bold text-slate-800 dark:text-slate-200">${s.nama}</td>
                                        <td class="py-3 px-4">${s.kategori}</td>
                                        <td class="py-3 px-4 text-right font-black text-emerald-600 text-sm">${s.stokKg.toLocaleString('id-ID')} ${s.satuan}</td>
                                        <td class="py-3 px-4 text-right text-slate-400">${s.batasMinimum} ${s.satuan}</td>
                                        <td class="py-3 px-4 text-right font-semibold">Rp ${s.biayaPerKg.toLocaleString('id-ID')} / ${s.satuan}</td>
                                        <td class="py-3 px-4 text-center">
                                            <span class="px-2 py-0.5 rounded text-[10px] font-bold ${s.stokKg > s.batasMinimum ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}">
                                                ${s.stokKg > s.batasMinimum ? 'Aman' : 'Perlu Re-Stok'}
                                            </span>
                                        </td>
                                        <td class="py-3 px-4 text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <button onclick="PakanKesehatanModule.openModalEditStokPakan('${s.id}')" title="Ubah Stok/Bahan" class="p-1.5 hover:bg-amber-50 dark:hover:bg-amber-950/50 rounded-lg text-amber-600">
                                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                                </button>
                                                <button onclick="PakanKesehatanModule.deleteStokPakan('${s.id}')" title="Hapus Bahan Pakan" class="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-950/50 rounded-lg text-rose-600">
                                                    <i data-lucide="trash-2" class="w-4 h-4"></i>
                                                </button>
                                            </div>
                                        </td>
