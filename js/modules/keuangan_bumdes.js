...
const KeuanganBumdesModule = {
    ...
    render(sub = null) {
        ...
        switch (this.currentSubMenu) {
            ...
            case "skema_kemitraan":
                return this.renderSkemaKemitraan();
            default:
                return this.renderBukuKas();
        }
    },
                                    ...
                                    <tr class="hover:bg-slate-50/70 dark:hover:bg-slate-900/30">
                                        ...
                                        <td class="py-3 px-4 text-center">
                                            <div class="flex items-center justify-center gap-1">
                                                <button onclick="KeuanganBumdesModule.cetakKwitansiKas('${k.id}')" title="Cetak Kwitansi Bukti Kas Resmi" class="p-1.5 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 rounded-lg text-emerald-600">
    ...
    terbilang(nominal) {
        ...
        if (n < 100) return this.terbilang(Math.floor(n / 10)) + " Puluh" + (n % 10 !== 0 ? " " + this.terbilang(n % 10) : "");
        if (n < 200) return "Seratus" + (n - 100 !== 0 ? " " + this.terbilang(n - 100) : "");
        if (n < 1000) return this.terbilang(Math.floor(n / 100)) + " Ratus" + (n % 100 !== 0 ? " " + this.terbilang(n % 100) : "");
        if (n < 2000) return "Seribu" + (n - 1000 !== 0 ? " " + this.terbilang(n - 1000) : "");
        if (n < 1000000) return this.terbilang(Math.floor(n / 1000)) + " Ribu" + (n % 1000 !== 0 ? " " + this.terbilang(n % 1000) : "");
        if (n < 1000000000) return this.terbilang(Math.floor(n / 1000000)) + " Juta" + (n % 1000000 !== 0 ? " " + this.terbilang(n % 1000000) : "");
        if (n < 1000000000000) return this.terbilang(Math.floor(n / 1000000000)) + " Miliar" + (n % 1000000000 !== 0 ? " " + this.terbilang(n % 1000000000) : "");
        return "Angka Terlalu Besar";
    },

    // --- CETAK KWITANSI BUKTI KAS MASUK / KELUAR RESMI ---
    cetakKwitansiKas(id) {
        const trx = Store.getKeuangan().find(k => k.id === id);
        if (!trx) {
            App.showToast("Data transaksi tidak ditemukan!", "error");
            return;
        }

        const pengaturan = Store.getPengaturan ? Store.getPengaturan() : {};
        const namaBadan = pengaturan.namaBadanUsaha || "BUMKal Lumbung Pangan Mataram";
        const desa = pengaturan.desa || "Kalurahan Pleret";
        const bendahara = "HAWIN (Bendahara BUMKal)";
        const direktur = "Direktur Utama BUMKal";
        const penyetor = trx.user || (trx.tipe === "masuk" ? "Penyetor / Rekanan" : "Penerima Uang");

        const noBukti = `${trx.tipe === 'masuk' ? 'BKM' : 'BKK'}/${trx.tgl.replace(/-/g, '/')}/${trx.id.slice(-4).toUpperCase()}`;
        const terbilangStr = this.terbilang(trx.nominal) + " Rupiah";

        const printWin = window.open("", "_blank");
        if (!printWin) {
            App.showToast("Gagal membuka jendela cetak. Pastikan pop-up diizinkan di browser Anda.", "warning");
            return;
        }
        ...
        printWin.document.write(`
            ...
            <head>
                <meta charset="UTF-8">
                <title>Kwitansi ${noBukti}</title>
                <style>
                    body { font-family: 'Arial', sans-serif; padding: 25px; color: #1e293b; max-width: 800px; margin: 0 auto; }
                    .header { border-bottom: 3px double #0f172a; padding-bottom: 12px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; }
                    .header-brand { display: flex; align-items: center; gap: 15px; }
                    .logo-box { width: 50px; height: 50px; background: #059669; color: white; border-radius: 8px; font-weight: 900; font-size: 22px; display: flex; align-items: center; justify-content: center; }
                    .company-name { font-size: 16px; font-weight: bold; text-transform: uppercase; color: #047857; margin: 0; }
                    .company-sub { font-size: 11px; color: #475569; margin: 2px 0 0 0; }
                    .voucher-title { text-align: right; }
                    .voucher-title h2 { margin: 0; font-size: 16px; font-weight: 900; color: ${trx.tipe === 'masuk' ? '#047857' : '#be123c'}; letter-spacing: 1px; }
                    .voucher-title div { font-size: 11px; font-family: monospace; font-weight: bold; color: #64748b; margin-top: 3px; }
                    .receipt-body { border: 1px solid #cbd5e1; border-radius: 12px; padding: 20px; background: #f8fafc; }
                    .row { display: flex; margin-bottom: 12px; font-size: 13px; line-height: 1.5; }
                    .row-label { width: 170px; color: #475569; font-weight: 600; flex-shrink: 0; }
                    .row-dots { width: 15px; text-align: center; }
                    .row-value { flex: 1; font-weight: 600; color: #0f172a; }
                    .terbilang-box { background: #e2e8f0; padding: 8px 12px; border-radius: 6px; font-style: italic; font-weight: bold; color: #1e293b; border-left: 4px solid ${trx.tipe === 'masuk' ? '#059669' : '#e11d48'}; }
                    .amount-badge { display: inline-block; background: ${trx.tipe === 'masuk' ? '#dcfce7' : '#ffe4e6'}; color: ${trx.tipe === 'masuk' ? '#166534' : '#9f1239'}; border: 2px solid ${trx.tipe === 'masuk' ? '#86efac' : '#fca5a5'}; padding: 6px 16px; border-radius: 8px; font-size: 18px; font-weight: 900; font-family: monospace; }
                    .sig-section { margin-top: 30px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; text-align: center; font-size: 12px; }
                    .sig-box { display: flex; flex-direction: column; justify-content: space-between; height: 130px; padding: 10px; border-radius: 8px; border: 1px dashed #cbd5e1; background: white; }
                    .sig-role { font-weight: bold; color: #475569; }
                    .sig-name { font-weight: bold; color: #0f172a; text-decoration: underline; }
                    .footer-note { font-size: 10px; color: #94a3b8; margin-top: 25px; text-align: center; border-top: 1px dashed #e2e8f0; padding-top: 10px; }
                    @media print {
                        .no-print { display: none !important; }
                        body { padding: 0; }
                        .receipt-body { background: white; border: 1px solid #000; }
                    }
                </style>
            </head>
            <body>
                <div class="header">
                    <div class="header-brand">
                        ...
                        <div>
                            <div class="company-name">${namaBadan}</div>
                            <div class="company-sub">${desa}, Kapanewon Pleret, Bantul, D.I. Yogyakarta</div>
                            <div class="company-sub">Unit Usaha Lumbung Ternak & Pengolahan Pupuk Organik</div>
                        </div>
                    </div>
                    <div class="voucher-title">
                        <h2>${trx.tipe === 'masuk' ? 'KWITANSI KAS MASUK (BKM)' : 'KWITANSI KAS KELUAR (BKK)'}</h2>
                        <div>NO: ${noBukti}</div>
                    </div>
                </div>
                ...
                <div class="receipt-body">
                    <div class="row">
                        <div class="row-label">${trx.tipe === 'masuk' ? 'Telah Diterima Dari' : 'Dibayarkan Kepada'}</div>
                        <div class="row-dots">:</div>
                        <div class="row-value">${penyetor}</div>
                    </div>
                    <div class="row">
                        <div class="row-label">Uang Sejumlah (Rupiah)</div>
                        <div class="row-dots">:</div>
                        <div class="row-value">
                            <div class="terbilang-box">### ${terbilangStr} ###</div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="row-label">Untuk Keperluan / Uraian</div>
                        <div class="row-dots">:</div>
                        <div class="row-value">${trx.keterangan || '-'} [Kategori: ${trx.kategori}]</div>
                    </div>
                    <div class="row">
                        <div class="row-label">Metode Pembayaran</div>
                        <div class="row-dots">:</div>
                        <div class="row-value">${trx.metode || 'Tunai / Transfer Bank'}</div>
                    </div>
                    <div class="row" style="margin-top: 16px; align-items: center;">
                        <div class="row-label" style="font-size: 14px;">TOTAL NOMINAL</div>
                        <div class="row-dots">:</div>
                        <div class="row-value">
                            <div class="amount-badge">Rp ${trx.nominal.toLocaleString('id-ID')},-</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="sig-section">
                    <div class="sig-box">
                        <div class="sig-role">${trx.tipe === 'masuk' ? 'Penyetor / Rekanan' : 'Penerima Uang'}</div>
                        <div>(...................................)</div>
                    </div>
                    <div class="sig-box">
                        <div class="sig-role">Bendahara BUMKal</div>
                        <div class="sig-name">${bendahara}</div>
                    </div>
                    <div class="sig-box">
                        <div class="sig-role">Menyetujui,<br>Direktur Utama BUMKal</div>
                        <div class="sig-name">${direktur}</div>
                    </div>
                </div>

                <div class="footer-note">
                    Dokumen ini dicetak sah dari Aplikasi Lumbung Ternak BUMKal LPM Pleret pada ${new Date().toLocaleString('id-ID')} • Bukti Transaksi Resmi.
                </div>

                <div class="no-print" style="margin-top: 25px; text-align: center;">
                    <button onclick="window.print()" style="padding: 10px 24px; font-size: 13px; font-weight: bold; background: #059669; color: white; border: none; border-radius: 8px; cursor: pointer; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1);">
                        🖨️ Cetak Kwitansi Sekarang (PDF / Printer)
                    </button>
                </div>
            </body>
            </html>
    ...
    // --- 6. SKEMA KEMITRAAN, GADUHAN & BAGI HASIL ---
    renderSkemaKemitraan() {
        const mitras = Store.getKemitraan ? Store.getKemitraan() : [];
        let totalModal = 0;
        let totalEkor = 0;
        let totalEstLaba = 0;

        mitras.forEach(m => {
            totalModal += (m.modalAwal || 0);
            totalEkor += (m.populasiEkor || 0);
            totalEstLaba += (m.estimasiLabaKotor || 0) - (m.biayaOperasional || 0);
        });

        return `
        <div class="space-y-6 pb-12">
            <!-- Header Banner -->
            <div class="bg-gradient-to-r from-amber-600 via-yellow-600 to-emerald-700 rounded-3xl p-6 sm:p-8 text-white shadow-xl relative overflow-hidden">
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-semibold uppercase tracking-wider mb-2">
                            <i data-lucide="handshake" class="w-3.5 h-3.5"></i> Kolaborasi Ekonomi Kerakyatan
                        </div>
                        <h1 class="text-2xl sm:text-3xl font-black tracking-tight">Skema Kemitraan & Bagi Hasil Ternak</h1>
                        <p class="text-amber-100 text-xs sm:text-sm mt-1 max-w-2xl">
                            Transparansi skema gaduhan domba, pembagian laba bersih antara BUMKal LPM Pleret dan warga/investor mitra secara akuntabel.
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <button onclick="KeuanganBumdesModule.openModalTambahMitra()" class="px-4 py-2.5 bg-white text-amber-800 hover:bg-amber-50 rounded-xl font-bold text-xs shadow-lg transition flex items-center gap-2 active:scale-95">
                            <i data-lucide="user-plus" class="w-4 h-4"></i> Daftarkan Mitra Baru
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <span class="text-xs font-semibold text-slate-500">Total Mitra Terdaftar</span>
                    <div class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-black text-slate-900 dark:text-white">${mitras.length}</span>
                        <span class="text-xs text-slate-500">mitra aktif</span>
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <span class="text-xs font-semibold text-slate-500">Populasi Gaduhan</span>
                    <div class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-black text-emerald-600 dark:text-emerald-400">${totalEkor}</span>
                        <span class="text-xs text-slate-500">ekor domba</span>
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <span class="text-xs font-semibold text-slate-500">Total Modal Kelolaan</span>
                    <div class="mt-2 flex items-baseline gap-2">
                        <span class="text-xl font-black text-amber-600 dark:text-amber-400">Rp ${totalModal.toLocaleString('id-ID')}</span>
                    </div>
                </div>
                <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <span class="text-xs font-semibold text-slate-500">Proyeksi Laba Bersih</span>
                    <div class="mt-2 flex items-baseline gap-2">
                        <span class="text-xl font-black text-slate-900 dark:text-white">Rp ${Math.max(0, totalEstLaba).toLocaleString('id-ID')}</span>
                    </div>
                </div>
            </div>

            <!-- SIMULATOR INTERAKTIF BAGI HASIL -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm space-y-4">
                <div class="flex items-center justify-between border-b pb-3 dark:border-slate-700">
                    <div>
                        <h3 class="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
                            <i data-lucide="calculator" class="w-4 h-4 text-emerald-600"></i> Simulator & Kalkulator Transparansi Bagi Hasil
                        </h3>
                        <p class="text-xs text-slate-400">Simulasikan margin laba dan rasio pembagian keuntungan per siklus panen.</p>
                    </div>
                    <span class="px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 text-xs font-bold">
                        BUMKal : Mitra
                    </span>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Populasi Ternak (Ekor)</label>
                        <input type="number" id="sim-ekor" value="10" min="1" oninput="KeuanganBumdesModule.hitungSimulasiBagiHasil()" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Harga Beli Bakalan (per Ekor)</label>
                        <input type="number" id="sim-beli" value="1300000" step="50000" oninput="KeuanganBumdesModule.hitungSimulasiBagiHasil()" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Biaya Pakan & Medis (per Ekor)</label>
                        <input type="number" id="sim-pakan" value="450000" step="10000" oninput="KeuanganBumdesModule.hitungSimulasiBagiHasil()" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Harga Jual Panen (per Ekor)</label>
                        <input type="number" id="sim-jual" value="2350000" step="50000" oninput="KeuanganBumdesModule.hitungSimulasiBagiHasil()" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold text-emerald-600">
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs pt-1">
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Persentase Bagi Hasil BUMKal (%)</label>
                        <input type="number" id="sim-pct-bumdes" value="60" min="0" max="100" oninput="KeuanganBumdesModule.syncSimPct('bumdes')" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold text-indigo-600">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-600 dark:text-slate-400 mb-1">Persentase Bagi Hasil Mitra (%)</label>
                        <input type="number" id="sim-pct-mitra" value="40" min="0" max="100" oninput="KeuanganBumdesModule.syncSimPct('mitra')" class="w-full p-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 font-bold text-emerald-600">
                    </div>
                </div>

                <!-- Simulation Result Card -->
                <div id="sim-result-box" class="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 text-xs">
                    <div>
                        <div class="text-slate-400 text-[11px]">Total Laba Bersih Siklus</div>
                        <div id="sim-laba-bersih" class="text-lg font-black text-slate-900 dark:text-white mt-0.5">Rp 6.000.000</div>
                        <div id="sim-roi" class="text-[10px] text-emerald-600 mt-0.5 font-bold">ROI: 34.3%</div>
                    </div>
                    <div class="p-2.5 rounded-lg bg-indigo-50/70 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60">
                        <div class="text-indigo-700 dark:text-indigo-300 font-bold text-[11px]">Hak BUMKal (60%)</div>
                        <div id="sim-hak-bumdes" class="text-base font-black text-indigo-900 dark:text-indigo-100 mt-0.5">Rp 3.600.000</div>
                    </div>
                    <div class="p-2.5 rounded-lg bg-emerald-50/70 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60">
                        <div class="text-emerald-700 dark:text-emerald-300 font-bold text-[11px]">Hak Mitra Peternak (40%)</div>
                        <div id="sim-hak-mitra" class="text-base font-black text-emerald-900 dark:text-emerald-100 mt-0.5">Rp 2.400.000</div>
                    </div>
                </div>
            </div>

            <!-- DAFTAR KONTRAK MITRA -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                    <div>
                        <h3 class="font-bold text-sm text-slate-900 dark:text-white">Daftar Kontrak Kemitraan & Gaduhan</h3>
                        <p class="text-[11px] text-slate-400">Pencatatan data mitra, skema rasio, dan status kontrak.</p>
                    </div>
                    <button onclick="KeuanganBumdesModule.openModalTambahMitra()" class="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs shadow transition flex items-center gap-1.5">
                    ...
                    <table class="w-full text-left border-collapse text-xs">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-900/60 text-[11px] font-extrabold uppercase tracking-wider text-slate-500 border-b border-slate-200 dark:border-slate-700">
                                <th class="p-3.5">No. Kontrak</th>
                                <th class="p-3.5">Nama Mitra</th>
                                <th class="p-3.5">Tipe Kemitraan</th>
                                <th class="p-3.5">Populasi Ekor</th>
                                <th class="p-3.5">Modal Awal</th>
                                <th class="p-3.5">Bagi Hasil (BUMKal : Mitra)</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td colspan="8" class="p-8 text-center text-slate-400">
                                    Belum ada data kontrak kemitraan. Silakan daftarkan mitra baru.
                                </td>
                            </tr>
                            ` : mitras.map(m => `
                            <tr class="border-b border-slate-100 dark:border-slate-700/60 hover:bg-slate-50">
                                <td class="p-3.5 font-mono font-bold text-slate-900 dark:text-white">${m.nomorKontrak}</td>
                                <td class="p-3.5">
                                    <div class="font-bold text-slate-900 dark:text-white">${m.namaMitra}</div>
                                    <div class="text-[10px] text-slate-400">${m.telepon || m.alamat || '-'}</div>
                                </td>
                                <td class="p-3.5">
                                    <span class="px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300 font-semibold text-[10px]">
                                        ${m.tipeMitra}
                                    </span>
                                </td>
                                <td class="p-3.5 font-bold text-slate-900 dark:text-white">${m.populasiEkor} ekor</td>
                                <td class="p-3.5 font-bold text-slate-700 dark:text-slate-300">Rp ${(m.modalAwal || 0).toLocaleString('id-ID')}</td>
                                <td class="p-3.5 font-bold text-emerald-600 dark:text-emerald-400">
                                    ${m.persenBumdes}% : ${m.persenMitra}%
                                </td>
                                <td class="p-3.5">
                                    <span class="px-2 py-0.5 rounded-full ${m.statusKontrak === 'Aktif' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300' : 'bg-slate-100 text-slate-600'} font-bold text-[10px]">
                                        ${m.statusKontrak}
                                    </span>
                                </td>
                                <td class="p-3.5 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <button onclick="KeuanganBumdesModule.deleteMitra('${m.id}')" title="Hapus Kontrak" class="p-1.5 text-slate-400 hover:text-rose-600 rounded-lg">
                                            <i data-lucide="trash-2" class="w-4 h-4"></i>
                                        </button>
                                    </div>
                                </td>
    ...
    syncSimPct(source) {
        const bumdesEl = document.getElementById("sim-pct-bumdes");
        const mitraEl = document.getElementById("sim-pct-mitra");
        if (!bumdesEl || !mitraEl) return;
        if (source === "bumdes") {
            const b = Math.max(0, Math.min(100, parseFloat(bumdesEl.value) || 0));
            mitraEl.value = (100 - b).toFixed(0);
        } else {
            const m = Math.max(0, Math.min(100, parseFloat(mitraEl.value) || 0));
            bumdesEl.value = (100 - m).toFixed(0);
        }
        this.hitungSimulasiBagiHasil();
    },

    hitungSimulasiBagiHasil() {
        const ekor = parseFloat(document.getElementById("sim-ekor")?.value) || 0;
        const beli = parseFloat(document.getElementById("sim-beli")?.value) || 0;
        const pakan = parseFloat(document.getElementById("sim-pakan")?.value) || 0;
        const jual = parseFloat(document.getElementById("sim-jual")?.value) || 0;
        const pctBumdes = parseFloat(document.getElementById("sim-pct-bumdes")?.value) || 60;
        const pctMitra = parseFloat(document.getElementById("sim-pct-mitra")?.value) || 40;

        const totalModal = ekor * (beli + pakan);
        const totalOmset = ekor * jual;
        const labaBersih = totalOmset - totalModal;

        const hakBumdes = labaBersih > 0 ? (labaBersih * pctBumdes / 100) : 0;
        const hakMitra = labaBersih > 0 ? (labaBersih * pctMitra / 100) : 0;
        const roi = totalModal > 0 ? ((labaBersih / totalModal) * 100).toFixed(1) : "0.0";

        const lbEl = document.getElementById("sim-laba-bersih");
        const roiEl = document.getElementById("sim-roi");
        const hbEl = document.getElementById("sim-hak-bumdes");
        const hmEl = document.getElementById("sim-hak-mitra");

        if (lbEl) lbEl.innerText = `Rp ${labaBersih.toLocaleString('id-ID')}`;
        if (roiEl) roiEl.innerText = `ROI: ${roi}%`;
        if (hbEl) hbEl.innerText = `Rp ${hakBumdes.toLocaleString('id-ID')}`;
        if (hmEl) hmEl.innerText = `Rp ${hakMitra.toLocaleString('id-ID')}`;
    },

    openModalTambahMitra() {
        const html = `
        <div class="p-6 space-y-5">
            <div class="flex items-center justify-between border-b pb-3 dark:border-slate-700">
                <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-xl bg-amber-100 dark:bg-amber-900/50 text-amber-700 flex items-center justify-center">
                        <i data-lucide="handshake" class="w-5 h-5"></i>
                    </div>
                    <div>
                        <h3 class="text-base font-bold text-slate-900 dark:text-white">Pendaftaran Kontrak Kemitraan</h3>
                        <p class="text-xs text-slate-500">Skema gaduhan ternak domba atau investasi modal warga.</p>
                    </div>
                </div>
                <button onclick="App.closeModal()" class="text-slate-400 hover:text-slate-600"><i data-lucide="x" class="w-5 h-5"></i></button>
            </div>

            <form onsubmit="KeuanganBumdesModule.submitMitra(event)" class="space-y-4 text-xs">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Nama Mitra / Warga *</label>
                        <input type="text" id="mtr-nama" required placeholder="Contoh: Bpk. Slamet Raharjo" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Tipe Kemitraan *</label>
                        <select id="mtr-tipe" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900">
                            <option value="Gaduhan Ternak">Gaduhan Ternak (Pemeliharaan Warga)</option>
                            <option value="Investor Permodalan">Investor Permodalan (Bagi Hasil Panen)</option>
                            <option value="Petani Pakan HPT">Petani Pakan HPT (Suplai Hijauan)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">NIK Mitra</label>
                        <input type="text" id="mtr-nik" placeholder="16 digit NIK KTP" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-mono">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Nomor WhatsApp / HP</label>
                        <input type="tel" id="mtr-hp" placeholder="08xxxxxxxxxx" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900">
                    </div>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Populasi Domba (Ekor)</label>
                        <input type="number" id="mtr-ekor" value="5" min="1" required class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Modal Awal (Rp)</label>
                        <input type="number" id="mtr-modal" value="6500000" step="100000" required class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold">
                    </div>
                    <div>
                        <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Rasio Bagi Hasil (BUMKal : Mitra)</label>
                        <div class="flex items-center gap-1">
                            <input type="number" id="mtr-pct-bumdes" value="60" min="0" max="100" class="w-1/2 p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold text-center">
                            <span>:</span>
                            <input type="number" id="mtr-pct-mitra" value="40" min="0" max="100" class="w-1/2 p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold text-center">
                        </div>
                    </div>
                </div>

                <div>
                    <label class="block font-semibold text-slate-700 dark:text-slate-300 mb-1">Alamat Domisili / Kandang Mitra</label>
                    <input type="text" id="mtr-alamat" placeholder="RT / Dusun di Kalurahan Pleret" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900">
                </div>
                ...
                <div class="pt-2 flex items-center justify-end gap-2">
                    ...
                    <button type="submit" class="px-5 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold shadow-md transition flex items-center gap-2">
                        <i data-lucide="check" class="w-4 h-4"></i> Daftarkan Kontrak
                    </button>
                </div>
            </form>
        </div>
    ...
    submitMitra(e) {
        e.preventDefault();
        const nama = document.getElementById("mtr-nama")?.value;
        const tipe = document.getElementById("mtr-tipe")?.value;
        const nik = document.getElementById("mtr-nik")?.value;
        const hp = document.getElementById("mtr-hp")?.value;
        const ekor = parseInt(document.getElementById("mtr-ekor")?.value) || 0;
        const modal = parseFloat(document.getElementById("mtr-modal")?.value) || 0;
        const pctBumdes = parseFloat(document.getElementById("mtr-pct-bumdes")?.value) || 60;
        const pctMitra = parseFloat(document.getElementById("mtr-pct-mitra")?.value) || 40;
        const alamat = document.getElementById("mtr-alamat")?.value;

        if (!nama) {
            App.showToast("Nama mitra wajib diisi!", "warning");
            return;
        }
        ...
        Store.addKemitraan({
            ...
            populasiEkor: ekor,
            modalAwal: modal,
            persenBumdes: pctBumdes,
            persenMitra: pctMitra,
            alamat: alamat
        });

        App.closeModal();
        App.showToast(`Kontrak kemitraan ${nama} berhasil disimpan!`, "success");
        if (typeof App.renderContent === "function") App.renderContent();
    },

    deleteMitra(id) {
        if (confirm("Hapus kontrak kemitraan ini?")) {
            Store.deleteKemitraan(id);
            App.showToast("Kontrak kemitraan dihapus.", "info");
            if (typeof App.renderContent === "function") App.renderContent();
        }
    }
};
