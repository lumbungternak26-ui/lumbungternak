...
const Store = {
    ...
    init() {
        ...
        const isInitialized = localStorage.getItem("kandang_system_initialized");
        if (!isInitialized && (!localStorage.getItem(STORAGE_KEYS.DOMBA) || !localStorage.getItem(STORAGE_KEYS.BATCHES_PENGGEMUKAN))) {
            console.log("Database kosong atau belum lengkap, memuat seed data awal...");
            this.seedInitialData();
            localStorage.setItem("kandang_system_initialized", "true");
        } else {
        ...
        if (this.safeGet(STORAGE_KEYS.TASKS, null) === null) {
            this.saveTasks(this.defaultTasksList());
        }
        if (this.safeGet(STORAGE_KEYS.AGENDAS, null) === null) {
        ...
        if (this.safeGet(STORAGE_KEYS.BREEDING_KAWIN, null) === null) {
            this.saveBreedingKawin(this.defaultBreedingKawin());
        }
        if (this.safeGet(STORAGE_KEYS.BREEDING_KELAHIRAN, null) === null) {
            this.saveBreedingKelahiran(this.defaultBreedingKelahiran());
        }
        if (this.safeGet(STORAGE_KEYS.BREEDING_KEMATIAN, null) === null) {
            this.saveBreedingKematian(this.defaultBreedingKematian());
        }
        this.getSuplier();
    ...
    seedInitialData() {
        ...
        localStorage.setItem(STORAGE_KEYS.BREEDING_KAWIN, JSON.stringify(this.defaultBreedingKawin()));
        localStorage.setItem(STORAGE_KEYS.BREEDING_KELAHIRAN, JSON.stringify(this.defaultBreedingKelahiran()));
        localStorage.setItem(STORAGE_KEYS.BREEDING_KEMATIAN, JSON.stringify(this.defaultBreedingKematian()));
    ...
    updateDomba(id, f) {
        ...
        if (i >= 0) {
            let updated = { ...l[i], ...f };
            // Sinkronisasi otomatis rekaman penimbangan awal jika tglMasuk atau bobotAwal diperbarui tanpa f.riwayatTimbang eksplisit
            if (!f.riwayatTimbang && (f.tglMasuk !== undefined || f.bobotAwal !== undefined)) {
                if (!Array.isArray(updated.riwayatTimbang) || updated.riwayatTimbang.length === 0) {
                    updated.riwayatTimbang = [{
                        tgl: updated.tglMasuk || new Date().toISOString().split('T')[0],
                        bobot: updated.bobotAwal !== undefined ? updated.bobotAwal : 0,
                        catatan: "Bobot awal masuk kandang"
                    }];
                } else {
                    updated.riwayatTimbang = [...updated.riwayatTimbang];
                    updated.riwayatTimbang[0] = {
                        ...updated.riwayatTimbang[0],
                        tgl: updated.tglMasuk || updated.riwayatTimbang[0].tgl,
                        bobot: updated.bobotAwal !== undefined ? updated.bobotAwal : updated.riwayatTimbang[0].bobot
                    };
                    updated.riwayatTimbang.sort((a, b) => new Date(a.tgl) - new Date(b.tgl));
                    if (updated.riwayatTimbang.length >= 2) {
                        const last = updated.riwayatTimbang[updated.riwayatTimbang.length - 1];
                        const prev = updated.riwayatTimbang[updated.riwayatTimbang.length - 2];
                        const diffDays = Math.max(1, Math.round((new Date(last.tgl) - new Date(prev.tgl)) / (1000 * 60 * 60 * 24)));
                        updated.adg = Math.round(((last.bobot - prev.bobot) * 1000) / diffDays);
                    }
                }
            }
            l[i] = updated;
            this.saveDomba(l);
            this.addLog(`Update domba ${l[i].eartag}: ${l[i].nama}`);
        }
    },
    ...
    calculateFCR(filterKandang = "all") {
        ...
        const cfg = this.getPengaturan ? this.getPengaturan() : {};
        if (totalGainKg <= 0) totalGainKg = Math.max(1, dombaList.length * 6);
        if (totalPakanKg <= 0) totalPakanKg = Math.round(totalGainKg * (cfg.targetFcr || 6.3));

        let fcr = Math.round((totalPakanKg / totalGainKg) * 10) / 10;
        if (cfg.targetFcr && cfg.targetFcr > 0 && logs.length === 0) {
            fcr = cfg.targetFcr;
        }
        return {
            fcr,
    ...
    calculateRealtimeHPP() {
        ...
        const cfg = this.getPengaturan ? this.getPengaturan() : {};
        const calculatedHpp = Math.round((avgKons * biayaKons) + (avgSil * biayaSil) + (avgHij * biayaHij));
        const hppPerEkorHari = (cfg.hppPakanPerEkorHari && cfg.hppPakanPerEkorHari > 0) ? cfg.hppPakanPerEkorHari : (cfg.biayaPakanHarianPerEkor || Math.max(2500, calculatedHpp));
        const totalHppHarianKandang = (cfg.totalBebanPakanHarian && cfg.totalBebanPakanHarian > 0) ? cfg.totalBebanPakanHarian : (hppPerEkorHari * dombaCount);
        const estimasiBulanan = (cfg.estimasiBiayaPakanBulan && cfg.estimasiBiayaPakanBulan > 0) ? cfg.estimasiBiayaPakanBulan : (totalHppHarianKandang * 30);
        ...
        return [
            {
                ...
                checklist: [
                    { text: "Timbang silase tebon jagung 500kg", teks: "Timbang silase tebon jagung 500kg", done: true },
                    { text: "Campur konsentrat & premix mineral secara homogen", teks: "Campur konsentrat & premix mineral secara homogen", done: true },
                    { text: "Uji aroma fermentasi harum asam segar", teks: "Uji aroma fermentasi harum asam segar", done: true }
                ]
            },
            {
                id: "tsk-2",
                ...
                checklist: [
                    { text: "Siapkan vaksin PMK suhu 2-8°C cold chain", teks: "Siapkan vaksin PMK suhu 2-8°C cold chain", done: true },
                    { text: "Injeksi 6 ekor domba di Kandang A", teks: "Injeksi 6 ekor domba di Kandang A", done: true },
                    { text: "Catat nomor batch vaksin di kartu rekam medis", teks: "Catat nomor batch vaksin di kartu rekam medis", done: false }
                ]
            },
            {
                id: "tsk-3",
                ...
                checklist: [
                    { text: "Ukur suhu inti tumpukan kompos (target 45-55°C)", teks: "Ukur suhu inti tumpukan kompos (target 45-55°C)", done: false },
                    { text: "Balik tumpukan menggunakan sekop mesin", teks: "Balik tumpukan menggunakan sekop mesin", done: false },
                    { text: "Siram larutan dekomposer EM4 jika terlalu kering", teks: "Siram larutan dekomposer EM4 jika terlalu kering", done: false }
                ]
            },
            {
                id: "tsk-4",
                ...
                checklist: [
                    { text: "Potong mepet tanah sisa 5cm dari pangkal", teks: "Potong mepet tanah sisa 5cm dari pangkal", done: false },
                    { text: "Angkut ke gudang chopper kandang", teks: "Angkut ke gudang chopper kandang", done: false }
                ]
            },
            {
                id: "tsk-5",
                ...
                checklist: [
                    { text: "Cetak mutasi rekening kas BPD DIY", teks: "Cetak mutasi rekening kas BPD DIY", done: true },
                    { text: "Cocokkan kuitansi kasir dengan buku kas digital", teks: "Cocokkan kuitansi kasir dengan buku kas digital", done: true },
                    { text: "Verifikasi tanda tangan Direktur BUMKal", teks: "Verifikasi tanda tangan Direktur BUMKal", done: false }
                ]
            }
        ];
    },
    ...
    addLahan(lahan) {
        ...
        if (!lahan.id) lahan.id = `lhn-${Date.now()}`;
        list.push(lahan);
        this.saveLahan(list);
        this.addLog(`Menambahkan plot lahan HPT: ${lahan.nama}`);
        return lahan;
    },
    updateLahan(id, fields) {
        const list = this.getLahan();
        const idx = list.findIndex(l => l.id === id);
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...fields };
            this.saveLahan(list);
            this.addLog(`Memperbarui plot lahan HPT: ${list[idx].nama}`);
            return list[idx];
        }
        return null;
    },
    deleteLahan(id) {
        ...
        this.saveLahan(filtered);
        if (found) this.addLog(`Menghapus plot lahan HPT: ${found.nama}`);
        return true;
    },

    // --- SKEMA KEMITRAAN & GADUHAN INVESTOR ---
    getKemitraan() { return this.safeGet(STORAGE_KEYS.KEMITRAAN, []); },
    saveKemitraan(list) { localStorage.setItem(STORAGE_KEYS.KEMITRAAN, JSON.stringify(list)); this.triggerAutoSync(); },
    addKemitraan(item) {
        const list = this.getKemitraan();
        if (!item.id) item.id = `mtr-${Date.now()}`;
        list.unshift(item);
        this.saveKemitraan(list);
        this.addLog(`Mencatat kemitraan baru: ${item.namaMitra} (${item.tipeKemitraan})`);
        return item;
    },
    updateKemitraan(id, fields) {
        const list = this.getKemitraan();
        const idx = list.findIndex(m => m.id === id);
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...fields };
            this.saveKemitraan(list);
            this.addLog(`Memperbarui data kemitraan: ${list[idx].namaMitra}`);
            return list[idx];
        }
        return null;
    },
    deleteKemitraan(id) {
        const list = this.getKemitraan().filter(m => m.id !== id);
        this.saveKemitraan(list);
        this.addLog(`Menghapus kemitraan: ${id}`);
        return true;
    },

    // --- DETEKSI LOW-STOCK ALERTS (PERINGATAN DINI ROP) ---
    getLowStockAlerts() {
        const alerts = [];
        const stokPakan = this.getStokPakan();
        stokPakan.forEach(s => {
            const min = parseFloat(s.batasMinimum) || 0;
            const cur = parseFloat(s.stokKg) || 0;
            if (cur <= min) {
                alerts.push({
                    id: s.id,
                    nama: s.nama,
                    kategori: s.kategori || "Pakan",
                    stokTerkini: cur,
                    batasMinimum: min,
                    satuan: s.satuan || "kg",
                    status: cur === 0 ? "Habis Total" : "Kritis (Menipis)",
                    urgensi: cur === 0 ? "danger" : "warning"
                });
            }
        });
        return alerts;
    ...
    getTasks() { 
        ...
        return t.map(task => {
            const dueDate = task.dueDate || task.tglDeadline || new Date().toISOString().split('T')[0];
            const tglDeadline = task.tglDeadline || dueDate;
            const assigneeNama = task.assigneeNama || task.assignee || "Petugas";
            const priority = task.priority || "normal";
            const unit = task.unit || "Kandang";
    ...
    addTask(task) {
        ...
        if (!task.dueDate && task.tglDeadline) task.dueDate = task.tglDeadline;
        if (!task.tglDeadline && task.dueDate) task.tglDeadline = task.dueDate;
        if (!task.assigneeNama) task.assigneeNama = "Petugas";
        if (!task.checklist) task.checklist = [];
        if (!task.comments) task.comments = [];
        list.unshift(task);
        this.saveTasks(list);
        this.addLog(`Menambahkan tugas baru: ${task.judul}`);
        return task;
    },
    updateTask(id, fields) {
        ...
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...fields };
            this.saveTasks(list);
            this.addLog(`Memperbarui tugas: ${list[idx].judul}`);
            return list[idx];
        }
        return null;
    },
    updateTaskStatus(id, newStatus) {
        ...
        if (t) {
            t.status = newStatus;
            this.saveTasks(list);
            this.addLog(`Memindahkan alur tugas "${t.judul}" ke ${newStatus}`);
            return true;
        }
        return false;
    },
    deleteTask(id) {
        ...
        if (t) this.addLog(`Menghapus tugas: ${t.judul}`);
        return true;
    },
    getSDM() { 
    ...
    defaultPengaturan() {
        return {
            ...
            hppPakanPerEkorHari: 7800,
            totalBebanPakanHarian: 0,
            targetFcr: 5.5,
            estimasiBiayaPakanBulan: 0,
            rataRataKoheHarianKg: 91,
            totalKapasitasLimbahTon: 5.1,
    ...
    getKopSuratHtml() {
        ...
        return `
            <div class="kop" style="text-align: center; border-bottom: 3px double #000; padding-bottom: 12px; margin-bottom: 25px;">
                ...
                <p style="margin: 0; font-size: 11px; font-style: italic; color: #333;">${cfg.alamatSekretariat || ''} • Telp/WA: ${cfg.nomorWhatsApp || cfg.kontakTelepon || '-'} • Email: ${cfg.emailResmi || cfg.emailLembaga || '-'}</p>
    ...
    defaultRolePermissions() {
        ...
        return {
            direksi: {
                ...
                deskripsi: "Direktur Utama, Manajer Usaha, Bendahara, dan Manajemen Operasional BUMKal (Super Admin).",
    ...
    getRolePermissions() {
        ...
        // Auto-merge missing roles or menus to ensure newly added routes are accessible
        let changed = false;
        Object.keys(def).forEach(rk => {
            if (!saved[rk]) {
                saved[rk] = def[rk];
                changed = true;
            } else if (Array.isArray(def[rk].allowedMenus)) {
                if (!Array.isArray(saved[rk].allowedMenus)) {
                    saved[rk].allowedMenus = [...def[rk].allowedMenus];
                    changed = true;
                } else {
                    def[rk].allowedMenus.forEach(m => {
                        if (!saved[rk].allowedMenus.includes(m)) {
                            saved[rk].allowedMenus.push(m);
                            changed = true;
                        }
                    });
                }
        ...
        if (changed) {
            localStorage.setItem(STORAGE_KEYS.ROLE_PERMISSIONS, JSON.stringify(saved));
        }
        return saved;
    },
    ...
    isRoleAllowed(role, menuKey) {
        ...
        // SUPER ADMIN / DIREKSI / ADMINISTRATOR BYPASS MUTLAK: Selalu izinkan 100% tanpa batasan
        if (normalizedRole === "direksi" || r.includes("admin") || r.includes("direktur") || r.includes("manajer")) {
            return true;
        }

        // Menu umum dan operasional lapangan mendesak selalu dibuka untuk semua staf
        if (["kanban", "siklus_breeding", "pertanian", "dashboard"].includes(menuKey)) {
            return true;
        }
    ...
    // --- SIKLUS BREEDING & REPRODUKSI (PEMBIAKAN) ---
    defaultBreedingKawin() {
        return [
            {
                id: "kwn-001",
                pejantanId: "dmb-1",
                pejantanEartag: "DMB-001",
                betinaId: "dmb-3",
                betinaEartag: "DMB-003",
                tglKawin: "2026-06-15",
                estimasiHpl: "2026-11-12",
                estimasiHPL: "2026-11-12",
                metode: "Alami (Koloni)",
                statusKebuntingan: "Bunting",
                kandangSekat: "Kandang B - Sekat Indukan 1",
                catatan: "Indukan DMB-003 kawin dengan pejantan Baron Dorper. Nafsu makan dan postur sangat baik.",
            ...
            },
            {
                ...
                pejantanEartag: "DMB-001",
                betinaId: "dmb-4",
                betinaEartag: "DMB-004",
                tglKawin: "2026-07-02",
                estimasiHpl: "2026-11-29",
                estimasiHPL: "2026-11-29",
                metode: "Inseminasi Buatan (IB)",
                statusKebuntingan: "Bunting",
                kandangSekat: "Kandang B - Sekat Indukan 2",
                catatan: "IB semen beku pejantan unggul Dorper. Pemeriksaan fisik dan palpasi menunjukkan fetus aktif.",
                createdAt: "2026-07-02T10:00:00.000Z"
            },
            {
                id: "kwn-003",
                pejantanId: "dmb-2",
                pejantanEartag: "DMB-002",
                betinaId: "dmb-7",
                betinaEartag: "DMB-007",
                tglKawin: "2026-02-22",
                estimasiHpl: "2026-07-22",
                estimasiHPL: "2026-07-22",
                metode: "Alami (Intensif)",
                statusKebuntingan: "Sudah Melahirkan",
                kandangSekat: "Kandang B - Sekat Bersalin",
                catatan: "Melahirkan 2 cempe kembar sehat (DMB-011 & DMB-012) pada 22 Juli 2026.",
                createdAt: "2026-02-22T09:15:00.000Z"
            }
        ];
    },

    defaultBreedingKelahiran() {
        return [
            {
                id: "lhr-001",
                kawinId: "kwn-003",
                ...
                catatan: "Proses partus lancar tanpa komplikasi. Kolostrum pertama telah disusui dengan baik.",
                createdAt: "2026-07-22T06:45:00.000Z"
            }
        ];
    },

    defaultBreedingKematian() {
        return [
            {
                id: "dth-001",
                dombaId: "hist-01",
                eartag: "DMB-HIST-01",
                kategoriTernak: "Cempe",
                tglKematian: "2026-08-14",
                penyebab: "Kedinginan / Hipotermia saat hujan ekstrem",
                tindakanBangkai: "Dikubur Sesuai SOP Biosecurity",
                petugas: "Wahyu Pratama",
                kandang: "Kandang B",
                catatan: "Kematian cempe umur 2 hari saat anomali cuaca dingin malam hari. Telah dipasang lampu pemanas infra-merah di sekat cempe.",
    ...
    getBreedingKawin() {
        const raw = localStorage.getItem(STORAGE_KEYS.BREEDING_KAWIN);
        let list = this.safeGet(STORAGE_KEYS.BREEDING_KAWIN, null);
        if (raw === null || list === null) {
            list = this.defaultBreedingKawin();
        }
        return list.map(item => {
            const hpl = item.estimasiHpl || item.estimasiHPL || "";
            return {
                ...item,
                estimasiHpl: hpl,
                estimasiHPL: hpl
    ...
    saveBreedingKawin(list) {
        localStorage.setItem(STORAGE_KEYS.BREEDING_KAWIN, JSON.stringify(list));
        this.triggerAutoSync();
    },
    addBreedingKawin(data) {
        const list = this.getBreedingKawin();
        const tglKawin = data.tglKawin || new Date().toISOString().split('T')[0];
        // Hitung estimasi HPL domba rata-rata 150 hari (5 bulan)
        let hpl = data.estimasiHpl || data.estimasiHPL;
        if (!hpl) {
            const d = new Date(tglKawin);
            d.setDate(d.getDate() + 150);
            hpl = d.toISOString().split('T')[0];
        }
        const item = {
            id: data.id || "kwn-" + Date.now(),
            pejantanId: data.pejantanId || "",
            pejantanEartag: data.pejantanEartag || "",
            betinaId: data.betinaId || "",
            betinaEartag: data.betinaEartag || "",
            tglKawin: tglKawin,
            estimasiHpl: hpl,
            estimasiHPL: hpl,
            metode: data.metode || "Alami (Koloni)",
            statusKebuntingan: data.statusKebuntingan || "Bunting",
            kandangSekat: data.kandangSekat || "",
            catatan: data.catatan || "",
            createdAt: new Date().toISOString()
        };
        list.unshift(item);
        this.saveBreedingKawin(list);
        this.addLog(`Mencatat perkawinan: ${item.betinaEartag} x ${item.pejantanEartag} (HPL: ${item.estimasiHpl})`);
        return item;
    },
    updateBreedingKawin(id, fields) {
        const list = this.getBreedingKawin();
        const idx = list.findIndex(k => k.id === id);
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...fields };
            this.saveBreedingKawin(list);
            this.addLog(`Update perkawinan: ${list[idx].betinaEartag}`);
            return list[idx];
        }
        return null;
    },
    deleteBreedingKawin(id) {
        const list = this.getBreedingKawin();
        const found = list.find(k => k.id === id);
        const filtered = list.filter(k => k.id !== id);
        this.saveBreedingKawin(filtered);
        if (found) this.addLog(`Hapus riwayat perkawinan: ${found.betinaEartag}`);
        return true;
    },

    getBreedingKelahiran() {
        const raw = localStorage.getItem(STORAGE_KEYS.BREEDING_KELAHIRAN);
        const list = this.safeGet(STORAGE_KEYS.BREEDING_KELAHIRAN, null);
        if (raw === null || list === null) {
            return this.defaultBreedingKelahiran();
        }
        return list;
    },
    saveBreedingKelahiran(list) {
        localStorage.setItem(STORAGE_KEYS.BREEDING_KELAHIRAN, JSON.stringify(list));
        this.triggerAutoSync();
    },
    addBreedingKelahiran(data) {
        const list = this.getBreedingKelahiran();
        const item = {
            id: data.id || "lhr-" + Date.now(),
            kawinId: data.kawinId || "",
            indukEartag: data.indukEartag || "",
            pejantanEartag: data.pejantanEartag || "",
            tglLahir: data.tglLahir || new Date().toISOString().split('T')[0],
            jumlahLahir: parseInt(data.jumlahLahir) || 1,
            jumlahHidup: parseInt(data.jumlahHidup) || 1,
            jumlahMati: parseInt(data.jumlahMati) || 0,
            kondisiInduk: data.kondisiInduk || "Sehat",
            cempeList: data.cempeList || [],
            kandang: data.kandang || "Kandang Breeding",
            sekat: data.sekat || "Sekat Bersalin",
            catatan: data.catatan || "",
            createdAt: new Date().toISOString()
        };
        list.unshift(item);
        this.saveBreedingKelahiran(list);

        if (item.kawinId) {
            this.updateBreedingKawin(item.kawinId, { statusKebuntingan: "Sudah Melahirkan" });
        }

        // Otomatis masukkan cempe yang hidup ke Master Domba
        if (Array.isArray(item.cempeList) && item.cempeList.length > 0) {
            const dombaList = this.getDomba();
            item.cempeList.forEach(c => {
                if (c.status !== "Mati") {
                    const newCempe = {
                        id: "dmb-" + Date.now() + "-" + Math.random().toString(36).substr(2, 4),
                        eartag: c.eartag || `CMP-${Date.now().toString().slice(-4)}`,
                        nama: c.nama || `Cempe ${item.indukEartag}`,
                        ras: c.ras || "Cross Dorper",
                        kategori: "Cempe",
                        kelamin: c.kelamin || "Jantan",
                        kandang: item.kandang,
                        sekat: item.sekat,
                        bobotAwal: parseFloat(c.bobotLahir) || 2.5,
                        bobotAwalKg: parseFloat(c.bobotLahir) || 2.5,
                        bobotTerkiniKg: parseFloat(c.bobotLahir) || 2.5,
                        tglMasuk: item.tglLahir,
                        status: "Sehat",
                        induk: item.indukEartag,
                        pejantan: item.pejantanEartag,
                        indukEartag: item.indukEartag,
                        pejantanEartag: item.pejantanEartag,
                        tglLahir: item.tglLahir,
                        adg: 0,
                        riwayatTimbang: [{ tgl: item.tglLahir, bobot: parseFloat(c.bobotLahir) || 2.5, catatan: "Bobot saat lahir" }],
                        catatan: `Kelahiran dari Induk ${item.indukEartag} & Pejantan ${item.pejantanEartag}. Bobot lahir: ${c.bobotLahir || 2.5} kg.`
                    };
                    dombaList.unshift(newCempe);
                }
            });
            this.saveDomba(dombaList);
        }

        this.addLog(`Mencatat kelahiran: Induk ${item.indukEartag} melahirkan ${item.jumlahHidup} cempe hidup.`);
        return item;
    },
    deleteBreedingKelahiran(id) {
        const list = this.getBreedingKelahiran();
        const found = list.find(l => l.id === id);
        const filtered = list.filter(l => l.id !== id);
        this.saveBreedingKelahiran(filtered);
        if (found) this.addLog(`Hapus riwayat kelahiran: ${found.indukEartag}`);
        return true;
    },
    ...
    // --- KEMITRAAN & SKEMA INVESTOR / GADUHAN ---
    getKemitraan() {
        return this.safeGet(STORAGE_KEYS.KEMITRAAN, []);
    },
    saveKemitraan(list) {
        localStorage.setItem(STORAGE_KEYS.KEMITRAAN, JSON.stringify(list));
        this.triggerAutoSync();
    },
    addKemitraan(data) {
        const list = this.getKemitraan();
        const item = {
            id: data.id || "mitra-" + Date.now(),
            nomorKontrak: data.nomorKontrak || `KTR/${new Date().getFullYear()}/${Date.now().toString().slice(-4)}`,
            namaMitra: data.namaMitra || "",
            tipeMitra: data.tipeMitra || "Gaduhan Ternak",
            nik: data.nik || "",
            telepon: data.telepon || "",
            alamat: data.alamat || "",
            tglMulai: data.tglMulai || new Date().toISOString().split('T')[0],
            tglSelesai: data.tglSelesai || "",
            populasiEkor: parseInt(data.populasiEkor) || 0,
            modalAwal: parseFloat(data.modalAwal) || 0,
            persenBumdes: parseFloat(data.persenBumdes) || 60,
            persenMitra: parseFloat(data.persenMitra) || 40,
            estimasiLabaKotor: parseFloat(data.estimasiLabaKotor) || 0,
            biayaOperasional: parseFloat(data.biayaOperasional) || 0,
            statusKontrak: data.statusKontrak || "Aktif",
            catatan: data.catatan || "",
            createdAt: new Date().toISOString()
        };
        list.unshift(item);
        this.saveKemitraan(list);
        this.addLog(`Menambah kemitraan: ${item.namaMitra} (${item.nomorKontrak})`);
        return item;
    },
    updateKemitraan(id, fields) {
        const list = this.getKemitraan();
        ...
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...fields };
            this.saveKemitraan(list);
            this.addLog(`Update kemitraan: ${list[idx].namaMitra}`);
            return list[idx];
        }
        return null;
    },
    ...
    resetToCleanZero() {
        const savedSDM = this.getSDM();
        const savedCurrentUser = this.getCurrentUser();
        const savedPengaturan = this.getPengaturan();
        const savedFirebase = localStorage.getItem(STORAGE_KEYS.FIREBASE_CONFIG);
        const savedTheme = localStorage.getItem(STORAGE_KEYS.THEME);

        // Kosongkan seluruh entitas operasional menjadi [] / 0
        localStorage.setItem(STORAGE_KEYS.DOMBA, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.PERIKSA_TIMBANG, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.KEUANGAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.LOG_PAKAN_HARIAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.KOHE_HARIAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BATCH_LIMBAH, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.PANEN_HPT, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.LAHAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.TASKS, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.AGENDAS, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BATCHES_PENGGEMUKAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BREEDING_KAWIN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BREEDING_KELAHIRAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BREEDING_KEMATIAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.KEMITRAAN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.GAJI_OPERASIONAL, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.BACKUP_SNAPSHOTS, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.LOGS, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.SUPLIER, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.PRESET_VAKSIN, JSON.stringify([]));
        localStorage.setItem(STORAGE_KEYS.DSS_PADES, JSON.stringify({
            tahunAnggaran: new Date().getFullYear(),
            targetSetoranPADes: 0,
            realisasiTerkini: 0,
            persentaseTercapai: 0,
            rekomendasiPanen: []
        }));

        // Nolkan populasi dan sekat di Master Kandang
        const currentKandang = this.safeGet(STORAGE_KEYS.MASTER_KANDANG, []);
        const zeroKandang = currentKandang.map(k => ({
            ...k,
            terisi: 0,
            sekatList: (k.sekatList || []).map(s => ({
                ...s,
                terisi: 0,
                eartags: []
            }))
        }));
        localStorage.setItem(STORAGE_KEYS.MASTER_KANDANG, JSON.stringify(zeroKandang));

        // Nolkan stok pakan dan pupuk
        const zeroStokPakan = this.getStokPakan().map(s => ({ ...s, stokKg: 0 }));
        localStorage.setItem(STORAGE_KEYS.STOK_PAKAN, JSON.stringify(zeroStokPakan));
        const zeroStokPupuk = this.getStokPupuk().map(p => ({ ...p, stok: 0, terjualBulanIni: 0 }));
        localStorage.setItem(STORAGE_KEYS.STOK_PUPUK, JSON.stringify(zeroStokPupuk));

        // Pulihkan akun user dan konfigurasi lembaga
        if (savedSDM && savedSDM.length > 0) localStorage.setItem(STORAGE_KEYS.SDM, JSON.stringify(savedSDM));
        if (savedCurrentUser) localStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(savedCurrentUser));
        if (savedPengaturan) localStorage.setItem(STORAGE_KEYS.PENGATURAN_SISTEM, JSON.stringify(savedPengaturan));
        if (savedFirebase) localStorage.setItem(STORAGE_KEYS.FIREBASE_CONFIG, savedFirebase);
        if (savedTheme) localStorage.setItem(STORAGE_KEYS.THEME, savedTheme);

        localStorage.setItem("kandang_system_initialized", "true");
        this.addLog("Reset total pabrik bersih: Seluruh data operasional di-nolkan. Akun user dipertahankan.");
