...
const PengaturanModule = {
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-2xl border border-rose-200 dark:border-rose-900/50 p-6 shadow-sm space-y-4">
                    ...
                    <div class="p-4 rounded-xl bg-rose-50/50 dark:bg-rose-950/20 border border-rose-100 dark:border-rose-900/40 text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <h4 class="font-bold text-rose-900 dark:text-rose-200">Reset Total ke Kondisi Bawaan Pabrik</h4>
                            <p class="text-slate-500 mt-0.5">Menghapus semua data yang diinput pengguna dan mengisi kembali dengan 12 ternak seed percontohan Lumbung Ternak Pleret.</p>
                        </div>
                        <button onclick="App.resetFactory()" class="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-md transition whitespace-nowrap">
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <button onclick="PengaturanModule.resetMasterPengaturan()" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-xl border border-rose-300 dark:border-rose-700 text-rose-700 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition">
                            <i data-lucide="rotate-ccw" class="w-4 h-4"></i> Reset Default Pabrik
                        </button>
    ...
    renderTabParameter(cfg) {
        return `
            <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm space-y-5">
                <div>
                    <h3 class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <i data-lucide="calculator" class="w-4 h-4 text-emerald-600"></i> Parameter Valuasi Aset Biologis & Finansial
                    </h3>
                    <p class="text-xs text-slate-500 mt-0.5">Konfigurasi nilai tukar daging, multiplier status ternak, target setoran PADes, dan ambang batas peringatan stok.</p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-xs">
                    ...
                    <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 space-y-2">
                        <label class="font-bold text-slate-700 dark:text-slate-300 block">Estimasi Biaya Pakan/Ekor/Hari (Rp)</label>
                        <p class="text-[11px] text-slate-500">Acuan HPP ransum pakan harian terpadu per ekor domba.</p>
                        <input type="number" id="cfg-biayaPakanHarianPerEkor" value="${cfg.biayaPakanHarianPerEkor || 7800}" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 font-bold">
                    </div>
    ...
    saveMasterPengaturanForm(showToast = true) {
        ...
        // 2. Parameter
        if (document.getElementById("cfg-hargaDagingHidupPerKg")) {
            ...
            updated.biayaPakanHarianPerEkor = getNum("cfg-biayaPakanHarianPerEkor", updated.biayaPakanHarianPerEkor);
            updated.safetyStockPakanKg = getNum("cfg-safetyStockPakanKg", updated.safetyStockPakanKg);
        }
    ...
    resetMasterPengaturan() {
        if (confirm("Apakah Anda yakin ingin memulihkan seluruh pengaturan ke kondisi default pabrik?")) {
            Store.resetPengaturan();
            App.showToast("Pengaturan sistem berhasil dipulihkan ke default pabrik!", "success");
            App.renderContent();
        }
    },
