...
const App = {
    ...
    login(userId) {
        ...
        if (this.currentRoute && Store.isRoleAllowed && !Store.isRoleAllowed(role, this.currentRoute)) {
            const allRoutes = [
                "dashboard", "scan_penimbang_cepat", "siklus_batch", "master_kandang", "data_ternak",
                "history_timbang", "cetak_stiker_qr", "import_csv", "input_pakan_harian", "scan_vaksin_medis",
                "stok_pakan_hpp", "rekam_medis", "limbah_organik", "buku_kas_bumdes", "laporan_rapat_evaluasi",
                "executive_dss_pades", "laporan", "penjualan_ternak", "gaji_operasional", "master_pengaturan",
                "bumdes_backup_purge", "firebase_sync", "master_preset_vaksin", "manajemen_user"
            ];
    ...
    resetFactory() {
        if (confirm("PERINGATAN: Seluruh data perubahan Anda akan dihapus dan dikembalikan ke data awal bawaan. Lanjutkan?")) {
            localStorage.clear();
            Store.init();
            this.closeModal();
